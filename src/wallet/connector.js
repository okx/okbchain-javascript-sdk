...
class Connector {
  ...
  get exAddress() {
    if(ZEROX.test(this.address)) {
      return crypto.convertHexToBech32(this.address);
    }
    return this.address;
  }
  ...
  async getAccounts() {
    ...
    return walletConnector.sendCustomRequest(params).then((res) => {
      const okbchainAccount = res.find((account) => {
        return EXCHAIN.test(account.address);
      });
      if (okbchainAccount) {
        address = okbchainAccount.address;
  ...
  async walletConnectInit(storageId) {
    ...
    walletConnector._clientMeta.name = 'ΟKB DEX';
    walletConnector._clientMeta.url = walletConnector._clientMeta.url.replace(/okb/i,'okb');
